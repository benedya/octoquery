...
import { startMysqlDatabase } from './setup/mysql-database'
import type { StartedTestDatabase } from './setup/test-database'

describe('MCP over MySQL (e2e)', () => {
  let database: StartedTestDatabase
  let app: McpTestApp

  beforeAll(async () => {
    database = await startMysqlDatabase([
  ...
  it('advertises one tool per configured database', async () => {
    ...
    expect(tools[0].description).toContain('MySQL')
  })
